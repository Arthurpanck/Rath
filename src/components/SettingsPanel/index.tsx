...
export function SettingsPanel({
  ...
  return (
    ...
    >
      <Group
        gap="xs"
        style={{ padding: "12px 16px", borderBottom: `1px solid ${MB_COLORS.border}` }}
      >
        <UnstyledButton
          onClick={onBack}
          aria-label="Retour"
      ...
      <Tabs value={tab} onChange={(v) => v && setTab(v)} variant="default">
        <Tabs.List grow style={{ padding: "0 8px" }}>
          {tabs.map((t) => (
            <Tabs.Tab key={t} value={t} style={{ fontWeight: 700, fontSize: 13 }}>
              {t}
            </Tabs.Tab>
          ))}
        </Tabs.List>
        ...
        <ScrollArea style={{ height: "calc(100vh - 190px)" }}>
          <Box style={{ padding: 16 }}>
            ...
            {tab === "Mise en forme" && (
              <FormatTab vizId={vizId} settings={settings} onChange={onChange} allCols={allCols} />
            )}
            {tab === "Plages" && <RangesTab settings={settings} onChange={onChange} />}
            {tab === "Couleurs" && (
              <ConditionalColorsTab
                vizId={vizId}
                settings={settings}
                onChange={onChange}
                allCols={allCols}

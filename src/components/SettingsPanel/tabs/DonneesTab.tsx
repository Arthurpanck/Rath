...
export function DonneesTab({
  ...
  if (vizId === "pie") {
    return (
      <Stack gap="md">
        <FieldSelect label="Mesure" value={settings.metrics?.[0] ?? activeMetrics[0]?.name} data={metricOptions} onPick={(v) => v && onChange({ metrics: [v] })} />
        <FieldSelect label="Dimension" value={settings.dimension} data={allCols} onPick={(v) => v && onChange({ dimension: v })} />
        <FieldSelect label="Anneau intérieur" value={settings.innerRing} data={dimOptions} onPick={(v) => onChange({ innerRing: v ?? undefined })} clearable placeholder="(optionnel)" />
        <FieldSelect label="Anneau extérieur" value={settings.outerRing} data={dimOptions} onPick={(v) => onChange({ outerRing: v ?? undefined })} clearable placeholder="(optionnel)" />
  ...
  if (vizId === "smartscalar") {
    ...
    return (
      <Stack gap="md">
        <FieldSelect label="Nombre principal" value={settings.metrics?.[0] ?? activeMetrics[0]?.name} data={metricOptions} onPick={(v) => v && onChange({ metrics: [v] })} />
        <Stack gap={6}>
  ...
  if (vizId === "sankey") {
    return (
      <Stack gap="md">
        <FieldSelect label="Source" value={settings.sourceField} data={dimOptions} onPick={(v) => v && onChange({ sourceField: v })} />
        <FieldSelect label="Destination" value={settings.targetField} data={dimOptions} onPick={(v) => v && onChange({ targetField: v })} />
        <FieldSelect label="Mesure" value={settings.metrics?.[0]} data={metricOptions} onPick={(v) => v && onChange({ metrics: [v] })} />
      </Stack>
    );
  }
  if (vizId === "pivot") {
    return (
      <Stack gap="md">
        <FieldSelect label="Lignes" value={settings.rowField} data={dimOptions} onPick={(v) => v && onChange({ rowField: v })} />
        <FieldSelect label="Colonnes" value={settings.colField} data={dimOptions} onPick={(v) => v && onChange({ colField: v })} />
        <FieldSelect label="Mesure" value={settings.metrics?.[0]} data={metricOptions} onPick={(v) => v && onChange({ metrics: [v] })} />
      </Stack>
    );
  }
  if (vizId === "map") {
    return (
      <Stack gap="md">
        <Select label="Carte par région" size="sm" data={MAP_REGIONS.map((r) => ({ value: r.value, label: r.label }))} value={settings.mapRegion} onChange={(v) => v && onChange({ mapRegion: v as MapRegion })} allowDeselect={false} comboboxProps={{ withinPortal: true }} />
        <FieldSelect label="Champ de région" value={settings.locationField} data={allCols} onPick={(v) => v && onChange({ locationField: v })} placeholder="Nom, code INSEE…" />
        <FieldSelect label="Champ de métrique" value={settings.metrics?.[0]} data={metricOptions} onPick={(v) => v && onChange({ metrics: [v] })} />
      </Stack>
    );
  }
  if (vizId === "scatter") {
    ...
    return (
      <Stack gap="md">
        <FieldSelect label="Taille des bulles" value={settings.bubbleField} data={metricOptions} onPick={(v) => onChange({ bubbleField: v ?? undefined })} clearable />
        <FieldSelect label="Axe X" value={xName} data={metricOptions} onPick={(v) => v && onChange({ metrics: [v, yName].filter(Boolean) })} />
        <FieldSelect label="Axe Y" value={yName} data={metricOptions} onPick={(v) => v && onChange({ metrics: [xName, v].filter(Boolean) })} />
      </Stack>
    );
  }
  if (vizId === "waterfall") {
    return (
      <Stack gap="md">
        <FieldSelect label="Axe X (dimension)" value={settings.dimension} data={allCols} onPick={(v) => v && onChange({ dimension: v })} />
        <FieldSelect label="Mesure" value={settings.metrics?.[0] ?? activeMetrics[0]?.name} data={metricOptions} onPick={(v) => v && onChange({ metrics: [v] })} />
        <Select label="Tri" data={SORT_OPTIONS} value={settings.sort} onChange={(v) => v && onChange({ sort: v as SortOrder })} allowDeselect={false} comboboxProps={{ withinPortal: true }} size="sm" />
      </Stack>
    );
  ...
  if (vizId === "treemap") {
    return (
      <Stack gap="md">
        <Stack gap={6}>
          <FieldSelect label="Grouping" value={settings.dimension} data={dimOptions} onPick={(v) => v && onChange({ dimension: v, hiddenValues: [] })} />
        ...
        />
        <FieldSelect label="Valeur" value={settings.metrics?.[0] ?? activeMetrics[0]?.name} data={metricOptions} onPick={(v) => v && onChange({ metrics: [v] })} />
      </Stack>
    );
  }
  ...
  return (
    <Stack gap="lg">
      <FieldSelect label={`${rowChart ? "Axe Y" : "Axe X"} (dimension)`} value={settings.dimension} data={allCols} onPick={(v) => v && onChange({ dimension: v })} />

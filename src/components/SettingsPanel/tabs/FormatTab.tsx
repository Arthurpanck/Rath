    ...
    <Stack gap="md">
      {(vizId === "scalar" || vizId === "object") && (
        <Select
          ...
          data={allCols}
          value={settings.scalarField ?? null}
          onChange={(v) => onChange({ scalarField: v ?? undefined })}
      ...
      {fmt.style === "currency" && (
        <>
          ...
          <Stack gap={4}>
            ...
            <Radio.Group
              value={fmt.currencyStyle}
              onChange={(v) => patch({ currencyStyle: v as CurrencyStyle })}
            >
              <Stack gap={4}>
          ...
          <Stack gap={4}>
            ...
            <Radio.Group
              value={fmt.currencyPlacement}
              onChange={(v) => patch({ currencyPlacement: v as CurrencyPlacement })}
            >
              <Stack gap={4}>
      ...
      <Select
        ...
        value={fmt.separator}
        onChange={(v) => v && patch({ separator: v as SeparatorStyle })}
        allowDeselect={false}
        comboboxProps={{ withinPortal: true }}
      />
      <NumberInput
        ...
        onChange={(v) => patch({ decimals: v === "" || v == null ? null : Number(v) })}
      />
      <NumberInput
        ...
        onChange={(v) => patch({ multiplyBy: v === "" || v == null ? null : Number(v) })}
      />
      <TextInput

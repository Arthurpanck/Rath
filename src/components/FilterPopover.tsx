    ...
    >
      {isId ? (
        <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
          <path
            d="M3 8.5l5.5-5.5H16a1 1 0 011 1v7.5L11.5 17z"
            stroke="currentColor"
            strokeWidth="1.5"
          />
...
function TableIcon() {
  return (
    <span
      style={{ display: "inline-flex", color: MB_COLORS.textTertiary, flexShrink: 0 }}
      aria-hidden
    >
      <svg width="16" height="16" viewBox="0 0 20 20" fill="none">
        <rect
...
export function FilterButton({
  ...
  const values = useMemo(
    () => (useCheckboxes && field ? distinctValues(dataset, field) : []),
    [dataset, field, useCheckboxes],
  );
  ...
  const visibleCols = dataset.cols.filter((c) =>
          ...
          {!field ? (
            <Box>
              ...
              <Stack gap={0} style={{ maxHeight: 380, overflowY: "auto", paddingBottom: 6 }}>
                ...
                {!collapsed && visibleCols.length === 0 && (
            ...
            <Box p="sm">
              ...
              <Menu shadow="md" width={250} position="bottom-start" withinPortal>
                <Menu.Target>
                  <Button
                    ...
                    aria-label="Opérateur de filtre"
                    rightSection={
              ...
              {!useCheckboxes && arity > 0 && (
                <Group grow gap="xs" mt="sm">
                  <TextInput
                    ...
                    placeholder={
                      arity === 2 ? "Min" : kind === "number" ? "Valeur" : "Saisir une valeur"
                    }
                    value={v1}
              ...
              <Divider my="sm" />
              <Stack gap="xs">
                <Button size="xs" radius="md" color="filter" onClick={() => apply(false)}>
                  Appliquer le filtre
                </Button>
                <Anchor
                  component="button"
                  ...
                  fz="xs"
                  fw={700}
                  style={{ color: "var(--mantine-color-filter-6)" }}
                  onClick={() => apply(true)}
                >
...
export function FilterBar({
  ...
  return (
    <Group
      gap={6}
      wrap="wrap"
      style={{ padding: "8px 16px", borderBottom: `1px solid ${MB_COLORS.border}`, flexShrink: 0 }}
    >
      {filters.map((f, i) => (
        <Group
          key={i}
          ...
          wrap="nowrap"
          style={{
            background: "var(--mantine-color-filter-0)",
            borderRadius: 999,
        ...
        >
          <Text
            ...
            style={{
              color: "var(--mantine-color-filter-7)",
              whiteSpace: "nowrap",
              overflow: "hidden",
              textOverflow: "ellipsis",
            }}
